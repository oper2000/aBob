...
do
	...
	if [ $found == 0 ]; then
		resultMissing+="$txt2verify<br>"
	fi
done

if test -n "$resultMissing"; then
  echo "<!DOCTYPE html><html><body><h1>Hybrid logcat verification report<br>&nbsp;Following missing from logcat</h1>" > ../Reports/latest/$1/logcatFailureReport.html
